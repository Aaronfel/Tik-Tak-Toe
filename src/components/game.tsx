...
const Game = ({xName, oName}: props) => {
  ...
  function winnerQ(p1: number, p2:number, p3:number) {
    let hasWinner = false
    if (iconsArray[p1].icon !== null && iconsArray[p2].icon !== null && iconsArray[p3].icon !== null) {
      if (iconsArray[p1].icon.type.name === iconsArray[p2].icon.type.name && iconsArray[p1].icon.type.name === iconsArray[p3].icon.type.name) {
        hasWinner = true
        winner = iconsArray[p1].icon.type.name === 'XIcon' ? `the winner is X`  : 'the winner is O'
      }
    }
    return hasWinner
  }
    ...
    <>
      {!game_overQ() ?
          ...
          <div className='flex justify-center mt-5'>
            <div className='bg-red-600 rounded-lg w-full lg:w-[50%] p-[20px] lg:p-[100px] h-[400px] lg:h-[650px] flex justify-center flex-wrap'>
                {iconsArray.map((item: any) => (
                  <div key={item.id} className='w-[100px] h-[120px] lg:w-[150px] lg:h-[150px]  border-4 border-white' onClick={() => putIcon(item.id)}>{item.icon}</div>  
                ))}
            </div>
          </div>
        </div>
        ...
        <div className='h-screen w-screen flex flex-col space-y-4 lg:space-y-10 justify-center items-center'>
          ...
          <button className='text-white bg-green-600 p-3 rounded-xl' onClick={() => window.location.reload()}> Play Again</button>
        </div>
      }
    </>
  );
